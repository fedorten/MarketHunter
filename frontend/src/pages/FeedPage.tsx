...
export function FeedPage({ onOpenAdvert }: Props) {
  ...
  return (
    <>
      <SearchFilters filters={filters} onChange={setFilters} onSubmit={() => setApplied(filters)} />
      <section className="section-head">
        <h1>Вещи рядом</h1>
        <p>Лента подстраивается под поиск и спокойно догружает объявления дальше.</p>
      </section>
      {items.length === 0 && !loading ? (
        <EmptyState title="Ничего не найдено" text="Попробуйте другой запрос или уберите часть фильтров." />
      ) : (
        <div className="adverts-grid">
          {items.map((advert, index) => (
            <AdvertCard key={`${advert.id}-${index}`} advert={advert} index={index} onOpen={onOpenAdvert} onLike={like} />
          ))}
        </div>
      )}
      <div ref={markerRef} className="load-marker">{loading ? 'Загружаем...' : ''}</div>
    </>
  )
}
